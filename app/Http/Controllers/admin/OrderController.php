...
class OrderController extends Controller {
	...
	public function createorder_form()
	{
		...
		$deliveryboys = $this->deliveryboy->getAllDeliveryBoys();
		return view('admin/createorder', array('users' => $users, 'ingredients' => $ingredients, 'categories' => $categories, 'deliveryboys' => $deliveryboys));
	}
	...
	{ 
		...
		if($valid->fails())
		{
			return redirect('admin/createorder')->withInput(Input::all())->with('error', $valid->errors());
		}
		else
		{
			Random :
			...
			if($customer_id == '')
			{
				...
				$branch = DB::table('branches')
							...
							->where('branches.id', Input::get('branch'))
							->where('language', 'en')
							->first();
			...
			if(count($items) == 0)
			{
				return redirect('admin/createorder')->withInput(Input::all())->with('item_error', trans('messages. Please select atleast one item to cart'));
			}
			else
			{
				...
				$this->order->accepted_at = date('Y-m-d H:i:s');
				$this->order->created_by = 'a';
				
				if(Input::get('order_type') == 'd' && Input::get('deliveryboy') != '')
				{ 
					$this->order->assigned_at = date('Y-m-d H:i:s');
				...
				foreach($items as $item)
				{
					...
					if($foodicsitem->foodics_id != '')
					{
						...
						if($item->options->is_execlusion)
						{
							...
							{
								$execlusion = DB::table('execlusions')->select('foodics_id')->where('id', $item['options']['execlusion_id'][$i])->first();
								$execlusions[] = array(
			...
			//echo '<pre>'; print_r($fields); exit;
				...
				return redirect('admin/orders')->with('success', trans('messages.Order Placed'));
			}
		}
	}
	...
	public function updateorder()
	{
		...
		if($valid->fails())
		{
			return redirect('admin/editorder/'.$order_id)->withInput(Input::all())->with('error', $valid->errors());
		}
		else
		{
			...
			if(Input::get('deliveryboy') != '')
			{
				DB::table('deliveryboy_request')->where('order_id', $order_id)->delete();
				DB::table('deliveryboy_request')->insert(['order_id' => $order_id, 'deliveryboy_id' => Input::get('deliveryboy'), 'status' => 'n', 'created_at' => date('Y-m-d H:i:s')]);
				$deliveryboy = $this->deliveryboy->getdeliveryboy(Input::get('deliveryboy'));
				$device_id = $deliveryboy->device_token;
				$branch = $this->branch->getbranch(Input::get('branch'));
			...
			if(Input::get('order_type') == 'd') {
		        ...
		        $delivery_time = new DateTime(date('Y-m-d H:i:s', strtotime($order_datetime))); 
	          	$start_time = new DateTime(date('Y-m-d H:i:s', strtotime($order_date. "+ ".$this->config_data['start_time']." mins"))); 
		        $end_time = new DateTime(date('Y-m-d H:i:s', strtotime($order_date. "+ ".$this->config_data['end_time']." mins"))); 
		        $branch_address = $branch->street.', '.$branch->city.', '.$branch->country; 
		        $fields = array('order' => array( 
		                         ...
		                         'cashOnDelivery' => true, 
		                         'cashOnDeliveryAmount' => round($total), 
		                         ), 
		                  'items' => [array('packingList' => $item_details, 'pickupPointId' => $branch->pickup_point_id, 'gpsLocation' => ['lat' => $branch->latitude, 'lng' => $branch->longitude], 'address' => $branch_address, 'title' => $branch->branch, 'contactName' => $branch->branch, 'phone' => $branch->mobile)], 
		                  'recipient' => array( 
		                            'firstName' => Input::get('customer_first_name'), 
		                            'lastName' => Input::get('customer_last_name'), 
		                            'mobile' => Input::get('customer_mobile'), 
		                            'deliveryPoint' => Input::get('address'), 
		                            'gpsLocation' => array('lat' => Input::get('latitude'), 'lng' => Input::get('longitude'))) 
		        ...
		        //echo '<pre>';  print_r($fields); exit; 
		        $ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, "https://test-api.dook.sa/api/Orders/".$order->dook_id."?access_token=".$this->config_data['dook_access_token']."");
			...
			return redirect('admin/orders')->with('success', trans('messages.Order Updated Successfully'));
		}
	}
	public function getbranch_deliveryboys()
	{
		...
		if(count($data))
		{
			foreach ($data as $row) 
			{
				$deliveryboys .= "<option value='".$row->id."'>".ucfirst($row->deliveryboy)."</option>";
			}
		}
	...
	*/
	...
	public function selectingredient()
	{
	   ...
	   $ingredien = view('admin/selectingredient', array('ingredients' => $data, 'vendor_item' => $vendor_item, 'size_list' => $size_list, 'execlusions' => $execlusions));
	...
	public function getcategory_items()
	{
		...
		if(count($data) > 0)
		{
			...
			foreach($data as $row)
			{
				...
				$link = ($row->is_ingredients == 1 || $row->is_size == 1) ? '<a href="javascript:selectingredient('.$row->id.');" class="get_ingredient">' : '<a href="#cartbox" onclick="addtocart('.$row->id.');">';
	...
	/* GET ORDER DETAILS FOR EDIT */
 	public function editorder($id) 
 	{
		...
		Session::put('flag', 1);
		return view('admin/editorder', array('order' => $order, 'data' => $data, 'ingredients' => $ingredients, 'branches' => $branches, 'categories' => $categories, 'deliveryboys' => $deliveryboys));
    }
	...
	public function update_orderstatus()
	{
		...
		if($status == 'd')
		{
			DB::table('orders')->where('id', $order_id)->update(['delivered_at' => date('Y-m-d H:i:s')]);
		}
		if($status == 'ca') {
			$header = array();
			$header[] = 'Content-type: application/json; charset=utf-8';
			$postfields = ['orderId' => $order->dook_id, 'dispatcherId' => $this->config_data['dook_company_id']];
			$postfields = json_encode($postfields);
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, "https://test-api.dook.sa/api/Orders/status/canceled?".$this->config_data['dook_access_token']);
			curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
			curl_setopt($ch, CURLOPT_HEADER, FALSE);
			curl_setopt($ch, CURLOPT_POST, TRUE);
	...
	public function assign_order($id)
	{
		...
		$order = DB::table('orders')
				 ...
				 ->select('user_addressbook.latitude', 'user_addressbook.longitude', 'branch_description.branch_name', 'branches.email', 'orders.dook_id', 'orders.branch_id')
				 ->where('orders.id', $id)
				 ->first();
		...
		if ( count($deliveryboys) ) {
			foreach ($deliveryboys as $deliveryboy) {
				$deliveryboyDookId[] = $deliveryboy->dook_id; 
			}
		}
		$deliveryboyDookId = json_encode($deliveryboyDookId);
		
		$header = array();
		$header[] = 'Content-type: application/json; charset=utf-8';
		
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'https://test-api.dook.sa/api/DriverGpsLocations?filter={"where":{"driverId":{"inq":'.$deliveryboyDookId.'}}}&access_token='.$this->config_data['dook_access_token']);
		curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
		curl_setopt($ch, CURLOPT_HEADER, FALSE);
		...
		if( count($result) ) {
			foreach ($result as $key => $value) {
				$deliveryboys[$key]->latitude = $value->gpsLocation->lat;
				$deliveryboys[$key]->longitude = $value->gpsLocation->lng;
			}
		}
		//echo "<pre>"; print_r($deliveryboys); exit;
		return view('admin/assign_deliveryboy', array('deliveryboys' => $deliveryboys, 'order_id' => $id));
	...
	public function assign_deliveryboy($id, $order_id)
	{
		$order = DB::table('orders')
				 ...
				 ->join('branch_description', 'branches.id', '=', 'branch_description.branch_id')
				 ->join('user_addressbook', 'orders.address_id', '=', 'user_addressbook.id')
				 ->select('user_addressbook.latitude', 'user_addressbook.longitude', 'branch_description.branch_name', 'branches.email','orders.dook_id')
				 ->where('orders.id', $order_id)
				 ->first();
		$deliveryboy = $this->deliveryboy->getdeliveryboy($id);
		DB::table('orders')->where('id', $order_id)->update(['deliveryboy_id' => $id, 'order_status' => 'as', 'assigned_at' => date('Y-m-d H:i:s')]);
		DB::table('deliveryboy_request')->where('order_id', $order_id)->delete();
		DB::table('deliveryboy_request')->insert(['order_id' => $order_id, 'deliveryboy_id' => $id, 'status' => 'n', 'created_at' => date('Y-m-d H:i:s')]); 
		$message = 'Hi '.$deliveryboy->deliveryboy_name.' Admin assigned a new Order for you. The branch details are, Shop Name: '.$order->branch_name.', Shop Email : '.$order->email.', Thanks & Regards, Shuneez team';
		$device_id = $deliveryboy->device_token;
		sendMessage($message, $device_id);
		
		if ( $deliveryboy->dook_id != '' )
			assignOrder($order->dook_id, $deliveryboy->dook_id, $this->config_data['dook_access_token']);
		
		return redirect('admin/orders')->with('success', trans('messages.Deliveryboy assigned successfully'));
	}
	...
	public function deleteorder($order_id)
	{
		...
		DB::table('order_itemdetails')->where('order_id', $order_id)->delete();
		DB::table('order_ingredientdetails')->where('order_id', $order_id)->delete();
		DB::table('delivery_boy_request_history')->where('order_id', $order_id)->delete();
		DB::table('deliveryboy_request')->where('order_id', $order_id)->delete();

		return redirect('admin/orders')->with('success', trans('messages.Order Delete'));
	}

	public function deliveryboyRatings()
	{
		$user_id = Input::get('user');
		$deliveryboy_id = Input::get('deliveryboy');
		$ratings = DB::table('orders')
					->join('deliveryboy_description', 'orders.deliveryboy_id', '=', 'deliveryboy_description.deliveryboy_id')
					->select('orders.id', 'orders.customer_first_name', 'orders.customer_last_name', 'deliveryboy_name as deliveryboy', 'order_key', 'deliveryboy_rating', 'deliveryboy_review')
					->where('dook_rating_id', '!=', '')
					->where('language', $this->current_language)
					->where(function($query) use($user_id, $deliveryboy_id){
						if ( $user_id != '' ) {
							$query->where('customer_id', $user_id);
						}
						if ( $deliveryboy_id != '' ) {
							$query->where('orders.deliveryboy_id', $deliveryboy_id);
						}
					})
					->paginate(10);
		...
		return view('admin/deliveryboy_ratings', ['ratings' => $ratings, 'users' => $users, 'deliveryboys' => $deliveryboys]);
	}
