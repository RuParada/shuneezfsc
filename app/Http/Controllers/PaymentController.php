...
class PaymentController extends Controller
{
     ...
     */
    public $merchantIdentifier = 'UolNguwn';
    
    /**
     * @var string your access code
     */
    public $accessCode         = 'mh4iX2OVs21V8iRtrfOQ';
    
    /**
     * @var string SHA Request passphrase
     */
    public $SHARequestPhrase   = 'PASS';
   ...
   // public $currency           = 'USD';
    ...
    public function getRedirectionData($paymentMethod) {
        ...
        $postData = array(
            'amount'              => $this->convertFortAmount(round(Session('payment.total')), "SAR"),
            'currency'            => strtoupper("SAR"),
            'merchant_identifier' => $this->merchantIdentifier,
            'access_code'         => $this->accessCode,
            'merchant_reference'  => $merchantReference,
            'customer_email'      => Session('payment.email'),
            //'customer_name'         => trim($order_info['b_firstname'].' '.$order_info['b_lastname']),
            'command'             => $this->command,
            'language'            => $this->language,
            'return_url'          => $this->getUrl('/payment_response'),
        );
	...
	{
		$_SESSION['total'] = Session('payment.total');
		return view('payfort/index');
	}
	
	public function payment_response()
	{
		$response_status = Input::get('response_message');
		$order_id = Session('payment.order_id');
		$fort_id = Input::get('fort_id');
		$payment_status = ($response_status == 'Success') ? 's' : 'f';
		DB::table('orders')->where('id', $order_id)->update(['payment_status' => $payment_status, 'transaction_id' => $fort_id]);
		$order = DB::table('orders')->where('id', $order_id)->first();
		Session::forget('payment');
		$mobile = valid_mobile($order->customer_mobile);
		if($response_status == 'Success')
		{
			$url = URL::to('update_address/'.$order->address_key.'/'.$order->order_key);
			$msg = 'Hi, Your order id - '.$order->invoice_number.' has been confirmed.';
