 ...
 //echo '<pre>'; print_r($orders); exit;
		if ( count($orders) ) {
			...
			//	print_r($end_time); exit;
				...
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, "https://test-api.dook.sa/api/FleetOwners/createOrder?access_token=".$this->config_data['dook_access_token']."");
...
//print_r($result); exit;
				if ( isset($result->result->order->id) ) {
					...
					$postfields = array(
						...
						$url =  "https://test-api.dook.sa/api/FleetOwners/599c55e06bf87d1200cf045b?access_token=".$this->config_data['dook_access_token']."";
	...
	public function updateDookOrderStatus()
	{
		...
		$response = json_decode($result['request']['body'], true);
		$dookResponse = json_encode($result);
		$status = $response['payload']['action'];
		$order_id = $response['payload']['id'];
