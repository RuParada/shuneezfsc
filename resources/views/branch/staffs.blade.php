...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <div class="box">                
            <div class="box-body">
               <p class="top_add_btn"><a href="{!! URL::to('branch/addstaff') !!}" class="btn btn-primary"><i class="fa fa-plus"></i><?php echo trans('messages.Add New Staff'); ?></a></p>
                <div class="row">
                    {!! Form::open(array('url' => 'branch/filterstaffs', 'method' => 'get')) !!}
                            ...
                            foreach ($staffs as $staff) {
                                ...
                                <tr>
                                   ...
                                   <td class="onoff" width="50">
										<input type="checkbox" name="active_inactive" <?php echo ($staff->status == 1) ? 'checked' : ''; ?> class="onoff_chck" id="<?php echo $staff->id; ?>" onclick="status_change(<?php echo $staff->id; ?>,<?php echo $staff->status; ?>)";>
                                        <label class="onoff_lbl" for="<?php echo $staff->id; ?>"></label>
                                    </td>
                                    <td class="action_btns">
                                        <a href="<?php echo URL::to('/branch/getstaff/'.$staff->id); ?>" class="edit_btn"><i class="fa fa-pencil"></i></a>
                                        <a href="#" class="delete_btn" onclick="delete_staff(<?php echo $staff->id; ?>)";><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                                <?php
                                $i++;
                ...
                if ($staffs->currentPage() == 1) {
                    ...
                    $count = $staffs->perPage() + $staffs->count();
                }
                // $count=($images->currentPage()==1)?$images->perPage():$images->perPage()+$images->count();
                if ($count > 0) {
                    $start = ($staffs->currentPage() == 1) ? 1 : $count - $staffs->count() + 1;
                } else {
                    $start = 0;
                }
...
<script>
    ...
    { 
        ...
        $.ajax({
            type: "GET",
            url: url + "/branch/change_staffstatus",
            dataType: "json",
    ...
    function delete_staff(id)
    {
        ...
        {
            window.location = url+'/branch/deletestaff/' + id;
        }
    }
</script>
@endsection
