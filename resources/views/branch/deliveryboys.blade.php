...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <div class="box">                
            <div class="box-body">
                <p class="top_add_btn"><a href="{!! URL::to('branch/adddeliveryboy') !!}" class="btn btn-primary"><i class="fa fa-plus"></i><?php echo trans('messages.Add New Deliveryboy'); ?></a></p>
                <div class="row">
                    {!! Form::open(array('url' => 'branch/filterdeliveryboy', 'method' => 'get')) !!}
                                ...
                                <tr>
                                    ...
                                    <td class="onoff" width="50">
                                        <?php if ($deliveryboy->is_delete == 0) {
                                            ?>
                                            <input type="checkbox" name="active_inactive" <?php echo ($deliveryboy->status == 1) ? 'checked' : ''; ?> class="onoff_chck" id="<?php echo $deliveryboy->id; ?>" onclick="status_change(<?php echo $deliveryboy->id; ?>,<?php echo $deliveryboy->status; ?>);">
                                    ...
                                    <td class="action_btns" width="80">
                                        <?php if ($deliveryboy->is_delete == 0) { ?>
                                            <a href="<?php echo URL::to('/branch/editdeliveryboy/'.$deliveryboy->id); ?>" class="edit_btn"><i class="fa fa-pencil"></i></a>
                                            <a href="#" class="delete_btn" onclick="delete_deliveryboy(<?php echo $deliveryboy->id; ?>)";><i class="fa fa-trash"></i></a>
                                        <?php } else { ?>
                                            <a href="#" onclick="restore_deliveryboy(<?php echo $deliveryboy->id; ?>);" title="Restore" class="edit_btn"><i class="fa fa-reply-all"></i></a>
                                        <?php } ?>
                                    </td>
                                </tr>
                                <?php
                ...
                if ($deliveryboys->currentPage() == 1) {
                    ...
                    $count = $deliveryboys->perPage() + $deliveryboys->count();
                }
                // $count=($images->currentPage()==1)?$images->perPage():$images->perPage()+$images->count();
                if ($count > 0) {
                    $start = ($deliveryboys->currentPage() == 1) ? 1 : $count - $deliveryboys->count() + 1;
                } else {
                    $start = 0;
                }
...
<script>
    function status_change(id, status)
    {
        ...
        $.ajax({
            type: "GET",
            url: url + "/branch/change_deliveryboystatus",
            dataType: "json",
