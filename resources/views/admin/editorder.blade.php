...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        {!! Form::open(array('url' => 'admin/updateorder', 'class' => 'form-horizontal', 'files' => 1)) !!}     
                ...
                <div class="box">       
                    ...
                    <div class="box-body">
                        <div class="form-group full_selectLists">
                            <label for="customer" class="col-sm-4 control-label"><?php echo trans('messages.Select Branch'); ?><span class="req">*</span></label>
                            <div class="col-sm-8">
                                <select name="branch" class="selectLists" onchange="getdeliveryboys()" id="branch">
                                 <option value=""><?php echo trans('messages.Select Branch'); ?></option>
                                    <?php 
                                    if(count($branches) > 0) { 
                                    foreach($branches as $branch) {
                                    ?>
                                        <option data-delivery=<?php echo $branch->delivery_fee; ?> value="<?php echo $branch->id; ?>" {{ $order->branch_id == $branch->id ? 'selected="selected"' : '' }}><?php echo $branch->branch; ?></option>  
                                    <?php } } ?>
                                </select>
                        ...
                        <div class="form-group full_selectLists">
                            <label for="customer" class="col-sm-4 control-label"><?php echo trans('messages.Select Deliverboy'); ?><span class="req">*</span></label>
                            <div class="col-sm-8">
                                   ...
                                   <?php
                                        if(count($deliveryboys) > 0)
                                        {
                                            foreach($deliveryboys as $deliveryboy) {
                                                $select = selectdrop($deliveryboy->id, $order->deliveryboy_id); 
                                        ?>
                                        <option <?php echo $select; ?> value="<?php echo $deliveryboy->id; ?>"><?php echo $deliveryboy->deliveryboy; ?></option>
                                    <?php } } ?>
                                </select>
                                @if(Session::has('error'))<p class="error_msg"><?php echo ($error->first('deliveryboy') != '') ? $error->first('deliveryboy') : ''; ?></p>@endif
                            </div>
                        </div>
                        <div class="form-group">
        ...
        <div id="cartdetails">
          <div class="col-md-5  bor_image well" id="cartbox">
            ...
            <div class="box-footer feeback_btns">
                ...
                <button type="button" onclick="window.location.href = '{!! URL::to('admin/orders') !!}'" class="btn pull-right"><i class="fa fa-chevron-left"></i><?php echo trans('messages.Cancel'); ?></button>
            </div><!-- box-footer -->
   ...
   function getdeliveryboys()
    {
        ...
        if(branch_id != '')
        {
            $.ajax({
                ...
                data: {'branch_id': branch_id, 'delivery_fee': delivery_fee},
                async: true,
    ...
    function updateqty(id)
    {
        ...
        if(qty != 0)
        {
            ...
            type : "GET",
            dataType : "json",
            url  : "<?php echo URL::to('admin/updateqty'); ?>",
    ...
    function delete_item(id)
    {
        if(confirm('Are you sure you want remove this item'))
        {
            ...
            type : "GET",
            dataType : "json",
            url  : "<?php echo URL::to('admin/delete_item'); ?>",
            data : {'id' : id},
            async: true,
            success: function(result) {
                        $("#item"+id).hide();
    ...
    $(document).ready(function() {
        $('input[type=radio][name=order_type]').change(function() {
            ...
            $.ajax({
                    ...
                    type: "GET",
                    dataType: "json",
                    url: "<?php echo URL::to('/admin/getbranch_delivery'); ?>",
